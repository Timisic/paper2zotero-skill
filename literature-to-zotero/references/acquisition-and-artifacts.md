...
4. **Capture and record.** Once the actual PDF is visible, `capture --current` reads its bytes without reloading the article. An HTML PDF viewer can embed the real file: the helper follows one observed same-origin PDF frame (such as Wiley's `pdfdirect`) and verifies the resulting bytes. Unrelated login/ad frames are excluded; signed addresses stay out of commands and public output. For an ordinary landing page, `capture --url` follows its article PDF. Always give the expected title and DOI; preserve version provenance from the source. Run the verifier and record only a verified file as `pdf_acquired`.
