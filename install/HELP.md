...
- 在配置窗口里按 **Ctrl+V** 粘贴授权码，看到 `******` 后按回车。也可以用右键菜单“粘贴”或 **Shift+Insert**。星号只表示收到输入，不显示授权码或其长度；按回车后会验证连接。
- 输入未正确粘贴时，向导会留在当前输入处，重新复制粘贴即可。个人文库 ID 会自动获取；连接失败会说明授权或网络原因，可以当场重试或稍后继续。
