...
默认 setup 依次是：**准备工具 → 连接 Zotero → 连接 OpenAlex（必配）→ 配置全文阅读（MinerU）→ 检查结果 → 更多配置（可选）**。OpenAlex 授权码必须填写并通过检索验证，基础配置才算完成。最后还会询问你是否想换用自己的总结提示词；选择查看时，只告诉你修改哪个文件，不会替你修改。最后的“更多配置”直接展开 Kimi WebBridge、Semantic Scholar、Crossref、Unpaywall 和电脑附件同步，按需填写即可。macOS 与 Windows 共用服务说明和验证规则；Windows 额外提供图形界面。

## 装好后怎么用

平时就在 Claude Code、Codex 或 Pi 的对话里使用。你负责提出需求、选择论文，助手负责后续处理。

1. **告诉助手你想读什么。** 例如：“帮我找 2024 年以来 AI 支持自我调节学习的研究，优先有真人干预实验，筛选 5 篇。”助手会给出题目、来源和入选理由。
2. **一次确认。** 例如：“选 1、2、4，保存到 Zotero 的‘AI 与学习’，允许上传 MinerU。”也可以说“不允许上传”。
...
中断后说“继续上次的任务”即可，不必重新找一遍。需要登录或某篇暂时获取失败时，会先交付已成功的部分，再说明下一步。

## 后台怎样处理

“前端”是你使用的 AI 助手；“后台”是本机脚本和按需调用的外部服务。
...
检索按可用配置调用 OpenAlex、Semantic Scholar 等来源；有公开 PDF 时直接下载，需要机构访问时才借助已登录浏览器。MinerU 负责把 PDF 转成文字，**阅读分析由当前 AI 助手完成**。不上传或转换失败时，仍可保存已核验的 PDF，并基于可读全文写笔记；Markdown 会标为不可用。Zotero 云端已入库和电脑附件已下载会分别报告。
